...
-- Gap teleport cooldown
local gapLastTeleportTime = 0
local GAP_TELEPORT_COOLDOWN = 0.5 -- seconds
...
TsunamiSection:Button({
    ...
    Locked = false,
    Callback = function()
        if tick() - gapLastTeleportTime < GAP_TELEPORT_COOLDOWN then
            WindUI:Notify({ Title = "Tsunami", Content = "Gap teleport cooldown", Icon = "alert-triangle", Duration = 2 })
            return
        end
        ...
        for _, g in ipairs(gaps) do
            ...
            if part then
                ...
                local lateral = (rel - look * proj).Magnitude
                if proj > 0 and lateral <= lateralThreshold then
                    local metric = proj + lateral * 0.1
        ...
        if best then
            gapLastTeleportTime = tick()
            local target = CFrame.new(best.Position + Vector3.new(0, 5, 0))
            local safe = findSafeCFrame(target, 6)
            local hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
            if not hrp then
                WindUI:Notify({ Title = "Tsunami", Content = "Character not ready", Icon = "alert-triangle", Duration = 3 })
                return
            ...
            pcall(function() hrp.CanCollide = false end)
            local ok, err = pcall(function()
                local tween = TweenService:Create(hrp, TweenInfo.new(0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { CFrame = safe })
                tween:Play()
                tween.Completed:Wait()
            end)
            pcall(function() hrp.CanCollide = prevCanCollide end)
            ...
            if ok then
                WindUI:Notify({ Title = "Tsunami", Content = "Teleported to next gap", Icon = "check", Duration = 3 })
            else
                WindUI:Notify({ Title = "Tsunami", Content = "Teleport failed: " .. tostring(err), Icon = "alert-triangle", Duration = 3 })
            end
        else
...
TsunamiSection:Button({
    ...
    Callback = function()
        ...
        if best then
            ...
            if ok then
                WindUI:Notify({ Title = "Tsunami", Content = "Teleported to previous gap", Icon = "check", Duration = 3 })
            else
                WindUI:Notify({ Title = "Tsunami", Content = "Teleport failed: " .. tostring(err), Icon = "alert-triangle", Duration = 3 })
            end
        else
...
UtilitiesSection:Toggle({
    Title = "God Mode",
    Desc = "Immune to the tsunami once or for more than 3 waves if the tsunami is very close to each other",
...
local SellAllBtn = UpgBase:Button({
    ...
    Callback = function()
        ...
        if currentTime - lastSellAllClick < 0.5 then
            -- Double click: sell
            pcall(function()
                game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("SellAll")
                    :InvokeServer()
            end)
            WindUI:Notify({
                Title = "Sold",
